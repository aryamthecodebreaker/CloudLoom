...
export default async function ConnectorsPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl p-8">
      <header>
        <h1 className="text-2xl font-extrabold tracking-tight text-wiz-navy">Connectors</h1>
      ...
      <ol className="mt-6 grid gap-4 md:grid-cols-3">
        {STEPS.map(([t, d], i) => (
          <li key={t} className="rounded-2xl border border-wiz-line bg-white p-5 shadow-card">
            <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-wiz-blue/10 text-xs font-bold text-wiz-blue">{i + 1}</span>
            <p className="mt-3 font-semibold text-wiz-navy">{t}</p>
            <p className="mt-1 text-xs leading-relaxed text-slate-500">{d}</p>
          </li>
        ))}
      </ol>

      <div className="mt-8 overflow-hidden rounded-2xl border border-wiz-line bg-white shadow-card">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-wiz-line">
            {accounts.map((a) => (
              <tr key={a.id} className="transition hover:bg-wiz-cloud/60">
                <td className="px-5 py-3.5 font-semibold text-wiz-navy">{a.name}</td>
