  ...
  return (
    <div className="mx-auto max-w-6xl p-8">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-wiz-navy">Vulnerabilities</h1>
      ...
      <div className="mt-6 space-y-4">
        {[...byCve.entries()].map(([cve, hits]) => {
          ...
          return (
            <article key={cve} className="rounded-2xl border border-wiz-line bg-white shadow-card">
              <div className="flex flex-wrap items-center gap-3 border-b border-wiz-line px-6 py-4">
                <span className="font-mono text-sm font-bold text-wiz-navy">{cve}</span>
                {v.exploitedInWild && (
              ...
              <div className="grid gap-5 p-6 md:grid-cols-5">
                ...
                <div className="md:col-span-3">
                  ...
                  <ul className="space-y-2">
                    {hits.map((h) => (
                      <li key={h.id + h.resource.id} className="flex items-center justify-between rounded-lg bg-wiz-cloud px-4 py-2.5 text-sm">
                        <span className="font-medium text-wiz-navy">{h.resource.name}</span>
                        <span className="text-xs text-slate-500">
                          {h.resource.provider} · {h.resource.type}
                          {(h.resource.isPublic || h.resource.hasSensitiveData) && (
                            <span className="ml-2 font-semibold text-wiz-pink">
