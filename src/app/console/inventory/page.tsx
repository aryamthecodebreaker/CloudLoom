...
export default async function InventoryPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl p-8">
      <header>
        <h1 className="text-2xl font-extrabold tracking-tight text-wiz-navy">Inventory</h1>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        {Object.entries(byProvider).map(([p, n]) => (
          <span key={p} className="rounded-xl border border-wiz-line bg-white px-4 py-2.5 text-sm font-semibold shadow-sm">
            {PROVIDER_LABELS[p] ?? p} <span className="ml-1 text-wiz-blue">{n}</span>
          </span>
        ))}
      </div>
      ...
      <div className="mt-4 overflow-hidden rounded-2xl border border-wiz-line bg-white shadow-card">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-wiz-line">
            {resources.map((r) => (
              <tr key={r.id} className="transition hover:bg-wiz-cloud/60">
                <td className="px-5 py-3.5">
                  <p className="font-semibold text-wiz-navy">{r.name}</p>
                ...
                <td className="px-4 py-3.5">
                  <span className={`font-semibold ${r._count.issues > 0 ? "text-wiz-blue" : "text-slate-400"}`}>
